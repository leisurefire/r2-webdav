...
export function bindMarkdownAiAssistant(
	...
	const openPanel = (
		...
		const saveSpaceSession = (assistant: string, original?: string, generated?: string, from?: number, to?: number) => {
			...
			const messages = [
				{ role: 'user' as const, content: userContent },
				{
					...
					content: assistant,
					thinking: t('思考完毕，已完成本次处理。', 'Thinking complete. This request has been processed.'),
				},
			];
		...
		const updateSpaceChange = (change: NoteChatChange | null) => {
			...
			spaceSave = (spaceSave ?? Promise.resolve())
				.then(() => api.updateNoteAiChat(options.noteId!, spaceSession!.id, { latestChange: change }))
				.then((saved) => {
					spaceSession = saved;
				});
		};
		...
		revertButton.addEventListener('click', () => {
			...
			spaceChange = spaceSession?.latestChange
				? { ...spaceSession.latestChange, status: 'reverted', updatedAt: new Date().toISOString() }
				: null;
			updateSpaceChange(spaceChange);
	...
	let unbindChat = () => {};
	const chatTrigger = editorRoot?.querySelector<HTMLButtonElement>('[data-note-ai-chat]');
	if (chatTrigger) chatTrigger.disabled = true;
	chatReady = import('./noteChat')
		.then(({ bindNoteContextChat }) => {
			if (destroyed) return;
			unbindChat = bindNoteContextChat(view, host, locale, options);
			if (chatTrigger?.isConnected) chatTrigger.disabled = false;
		})
		.catch(onError);
